...
typedef struct _my_datamodel_table {
    enum_mpsx_stdarg_integer_type_t int_type;
    int8_t bytes;
    int16_t bits;
...
{ MPSX_STDARG_INTEGER_IS_Normal,    4, 32},
{ MPSX_STDARG_INTEGER_IS_Long  ,    4, 32},
{ MPSX_STDARG_INTEGER_IS_LL    ,    8, 64},
{ MPSX_STDARG_INTEGER_IS_Ptr   ,    8, 64},
...
/* integer type is                ,bytes, bits */
{ MPSX_STDARG_INTEGER_IS_Normal,    8, 64},
{ MPSX_STDARG_INTEGER_IS_Long  ,    8, 64},
{ MPSX_STDARG_INTEGER_IS_LL    ,    8, 64},
{ MPSX_STDARG_INTEGER_IS_Ptr   ,    4, 64},
#elif defined(MY_IL32P32) /* IL32P32 standard 32-bit */ 
/* integer type is                ,bytes, bits */
{ MPSX_STDARG_INTEGER_IS_Normal,    4, 32},
{ MPSX_STDARG_INTEGER_IS_Long  ,    4, 32},
{ MPSX_STDARG_INTEGER_IS_LL    ,    8, 64},
{ MPSX_STDARG_INTEGER_IS_Ptr   ,    4, 32},
#else
#error undefined datamodel
#endif
{ MPSX_STDARG_INTEGER_IS_eot   ,   -1,  0}};
...
static int _own_vsnprintf(const mpsx_xtoa_output_method_t *, const char *const, va_list)
	__attribute__ ((optimize("Os")));

static mpsx_xtoa_output_method_t set_area_method(mpsx_xtoa_output_method_t m, char *const buf, const size_t max_len)
	__attribute__ ((optimize("Os")));

extern int mpsx_lite_vprintf(const char *const fmt, va_list ap)
	__attribute__ ((optimize("Os")));
extern int mpsx_lite_vsprintf(char *const buf, const char *const fmt, va_list ap)
	__attribute__ ((optimize("Os")));
extern int mpsx_lite_vsnprintf(char *const buf, const size_t max_length, const char *const fmt, va_list ap)
	__attribute__ ((optimize("Os")));


extern int mpsx_lite_putchar_cb_sprintf(int (*const putchar_cb)(int) , const char *const fmt, ...)
	__attribute__ ((optimize("Os")))
	__attribute__((format(printf,2,3)));
extern int mpsx_lite_putchar_cb_snprintf(int (*const putchar_cb)(int), const size_t max_length, const char *const *fmt, ...)
	__attribute__ ((optimize("Os")));
//	__attribute__((format(printf,3,4)));

extern int mpsx_lite_printf_init(int (*const putchar_cb_func)(const int asicccode))
	__attribute__ ((optimize("Os")));
extern int mpsx_lite_printf_change_putchar_callback(int (*const putchar_cb_func)(const int asicccode))
	__attribute__ ((optimize("Os")));
extern int mpsx_lite_printf(const char *const fmt, ...)
	__attribute__ ((optimize("Os")))
	__attribute__((format(printf,1,2)));
extern int mpsx_lite_snprintf(char *const buf, const size_t max_length, const char *const fmt,  ...)
	__attribute__ ((optimize("Os")));
...
static int _own_put_integer(void (*_putchar_cb)(int), unsigned long long llval, int radix, int length, char sign, const conversion_specifications_t *const specs_p, const int szoflimit, int *retlen_p)
{
    ...
    char buf[MPSX_XTOA_BIN64_BUFSZ];
...
/**
 * @fn int mpsx_vsnprintf_lite(void (*_putc_cb)(int), const size_t max_len, const char *const fmt, va_list ap)
...
int _mpsx_vsnprintf_lite(void (*_putc_cb)(int), const size_t max_len, const char *const fmt, va_list ap)
{
	...
	tmp = 0;
	specs.flags = 0;
	int_type = MPSX_STDARG_INTEGER_IS_Normal;
	...
	for( specs.flags = 0; (NULL != mpsx_strchr("'-+ #0", *fptr)); ++fptr) {
	    switch (*fptr) {
	    case '\'': {
	...
	if(*fptr == '*') {
	    ...
	    for(;mpsx_ascii_isdigit(*fptr); ++fptr) {
		length *= 10;
		length += mpsx_ctoi(*fptr);
	    }
	}
		...
		while (mpsx_ascii_isdigit(*fptr) ) {
		    precision *= 10;
		    precision += mpsx_ctoi(*fptr);
		    ++fptr;
		}
	    }
	}

	while (mpsx_strchr("hljzt", *fptr)) {
	    switch (*fptr++) {
            case 'h': {
		int_type = MPSX_STATIC_CAST(enum_mpsx_stdarg_integer_type_t, int_type-1);
		} break;
	    case 'l' : {
		int_type = MPSX_STATIC_CAST(enum_mpsx_stdarg_integer_type_t, int_type+1);
		} break;
	    case 'j': /*intmax   : long      */
            case 'z': /*size     : long      */
            case 't': /*ptrdiff  : long      */
		int_type=MPSX_STDARG_INTEGER_IS_Long;
		break;
	    default: 
		errno = EINVAL;
		return -1;
	...
	case 'd':
	    ...
	    int retlen = 0;
            const long long int lli = mpsx_get_va_list_signed(&ap, int_type);
	...
	case 'u': {
	    const int remain = (max_len == 0) ? 0 : (int)(max_len - lenofneeders);
	    int retlen = 0;
	    const unsigned int ui = (unsigned int)mpsx_get_va_list_unsigned(&ap, int_type);
	...
	case 'p':
	    length = datamodel_tab[MPSX_STDARG_INTEGER_IS_Ptr].bytes;
	    int_type = MPSX_STDARG_INTEGER_IS_Ptr;
	    sign = 0;
	    specs.f.zero_padding = 1;
	    specs.f.alternative = 1;
	case 'x': 
	...
	case 'c': {
	    const int code = (int)mpsx_get_va_list_signed(&ap, MPSX_STDARG_INTEGER_IS_Char);
	...
	case 's': {
	    const char *s = (char*)mpsx_get_va_list_pointer(&ap);
...
int _mpsx_vsprintf_lite(void (*_putc_cb)(int), const char *const fmt, va_list ap)
{
    const int ret = _mpsx_vsnprintf_lite(_putc_cb, 0, fmt, ap);
    return ret;
}
...
/**
 * @fn int _own_vsnprintf(const mpsx_xtoa_output_method_t *const method_p, const char *const fmt, va_list ap)
 * @brief fmt に従った文字列を生成しながら pufc_cbに指定されたコールバック関数を用いて出力します
 *	max_lenを超える場合は、処理を中止して -1 を戻します。
 * @param putc_cb 処理結果の文字列を出力するためのコールバック関数
 * @param max_len 処理結果を受ける最大長(\0)を含む。 0:最大長は無視
 ...
 * @retval -1 失敗
 **/
static int _own_vsnprintf(const mpsx_xtoa_output_method_t *const method_p, const char *const fmt, va_list ap)
{
    int result;
    int lenofneeders = 0;
    const char *fptr;
...
//	    xil_printf("%s : pre strings fini." EOL_CRLF, __func__);
	...
	if(1) { 
	    ...
	    for( specs.flags = 0; (NULL != mpsx_strchr( printf_flag_chars, *fptr)); ++fptr) {
		switch (*fptr) {
		    case '\'': 
			specs.f.thousand_group = 1;
			break;
	...
	if(*fptr == '*') {
	    length = (int)mpsx_get_va_list_signed( &ap, MPSX_STDARG_INTEGER_IS_Normal);
	    ++fptr;
	} else {
	    for(;mpsx_ascii_isdigit(*fptr); ++fptr) {
		length *= 10;
        ...
        if (*fptr == '.') {
            ...
            if (*fptr == '*') {
		++fptr;
		precision = (int)mpsx_get_va_list_signed(&ap, MPSX_STDARG_INTEGER_IS_Normal);
	    } else {
		while (mpsx_ascii_isdigit(*fptr) ) {
		    precision *= 10;
		    precision += mpsx_ctoi(*fptr);
	...
	if(1) {
	   ...
	   while (mpsx_strchr(fmt_length_modifier,*fptr)) {
		switch (*fptr++) {
		case 'h': 
		    int_type = MPSX_STATIC_CAST(enum_mpsx_stdarg_integer_type_t, int_type-1);
		    break;
		...
		case 'z': /* size : long long     */
		    int_type=MPSX_STDARG_INTEGER_IS_LL;
		    break;
		default: 
		    errno = EINVAL;
		    return -1;
		...
		mpsx_xtoa_output_method_t m = set_area_method( *method_p, bufp, remain);
		int retlen = 0;
		...
		result = mpsx_integer_to_string_with_format(&m, sign, ulli, 10, length, &specs, &retlen);
		if(result) {
		    errno = result;
		    return -1;
		}
...
//		xil_printf("%s : d i lenofneeders=%d retlen=%d" EOL_CRLF, __func__, lenofneeders, retlen); 
	    ...
	    case 'u': {
		const int remain = (int)(max_len - lenofneeders);
		const unsigned long long int ulli = mpsx_get_va_list_unsigned(&ap, int_type);
		mpsx_xtoa_output_method_t m = set_area_method( *method_p, bufp, remain);
		int retlen = 0;
...
//		xil_printf("%s : u lenofneeders=%d retlen=%d" EOL_CRLF, __func__, lenofneeders, retlen); 
		...
		result = mpsx_integer_to_string_with_format(&m, sign, ulli, 8, length, &specs, &retlen);
		if(result) {
		    errno = result;
		    return -1;
		}
...
//		xil_printf("%s : o lenofneeders=%d retlen=%d" EOL_CRLF, __func__, lenofneeders, retlen); 
		...
		result = mpsx_integer_to_string_with_format( &m, 0, ulli, 16, length, &specs, &retlen);
		if(result) {
		    errno = result;
		    return -1;
		}
...
static mpsx_xtoa_output_method_t _this_out_method =
    mpsx_xtoa_output_method_initializer_set_callback_func(NULL, 0); 
...
int mpsx_lite_printf_init(int (*const putchar_cb_func)(const int asicccode))
{
	...
	const mpsx_xtoa_output_method_t out_method = 
	    mpsx_xtoa_output_method_initializer_set_callback_func(putchar_cb_func, 0);
 ...
 * @brief mpsx_integer_to_string_with_fomat()のための標準出力関数
...
/**
 * @fn int mpsx_lite_printf_change_putchar_callback(const putchar_callback_ptr_t _putchar_cb);
 * @brief mpsx_lite_printf()が生成した文字列を指定された _putchar_cv で指定された関数に変更する
...
int mpsx_lite_printf_change_putchar_callback(int (*const putchar_cb_func)(const int asicccode))
{
    if( NULL == putchar_cb_func ) {
	const mpsx_xtoa_output_method_t out_method = 
	    mpsx_xtoa_output_method_initializer_set_callback_func( _this_putchar_stdout, 0); 
	_this_out_method = out_method;
    } else { 
	const mpsx_xtoa_output_method_t out_method = 
	    mpsx_xtoa_output_method_initializer_set_callback_func( putchar_cb_func, 0); 
...
{
    const mpsx_xtoa_output_method_t _method = 
	mpsx_xtoa_output_method_initializer_set_buffer( buf, max_length);
...
/**
 * @fn int mpsx_lite_vsprintf(char *const buf, const char *const fmt, va_list ap)
...
{
    const mpsx_xtoa_output_method_t _method = 
	mpsx_xtoa_output_method_initializer_set_buffer( buf, 0);
...
/**
 * @fn int mpsx_lite_printf(const char *const fmt, ...)
 * @brief printf()のオリジナルソースコードカスタマイズ版です。コード削減のために浮動小数点は対応していません。
 * 	出力は mpsx_lite_printf_change_putchar_callback()で変更します。
 * @param fmt 出力フォーマット
 * @param ... 可変引数
 * @retval -1 失敗
 * @retval 0以上　出力文字数
...
int mpsx_lite_printf(const char *const fmt, ...)
{
    ...
    va_start(ap, fmt);
    retval = mpsx_lite_vprintf(fmt, ap);
    va_end(ap);
...
int mpsx_lite_sprintf(char *const buf, const char *const fmt, ...)
{
    ...
    va_start(ap, fmt);
    retval = mpsx_lite_vsprintf( buf, fmt, ap);
    va_end(ap);

    return retval;
}

int mpsx_lite_snprintf(char *const buf, const size_t max_length, const char *const fmt, ...)
{
    int retval;
    va_list ap;
    ...
    va_start(ap, fmt);
    retval = mpsx_lite_vsnprintf( buf, max_length, fmt, ap);
    va_end(ap);
