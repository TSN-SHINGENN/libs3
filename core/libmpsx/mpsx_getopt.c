...
static int own_getopt_exec(mpsx_getopt_t * const __restrict self_p,
			   const int argc, const char *const argv[],
			   const char *const ostr);
static int own_getopt_long_exec(mpsx_getopt_t * const __restrict self_p,
				const enum_getopt_type_t getopt_type,
				const int argc, const char *const argv[],
				const char *const optstring,
				const mpsx_getopt_long_option_t * const lopts,
				int *const index_p);

/**
 * @fn mpsx_getopt_t mpsx_getopt_initializer(void)
 * @brief mpsx_getopt_t構造体インスタンスを初期化する為に初期値を代入します。
 * @return mpsx_getopt_t構造体初期値
 */
...
/**
 * @fn static int own_getopt_exec(mpsx_getopt_t * const self_p, int argc, char *argv[], const char *const ostr)
 * @brief コマンドライン引数解析を行う（内部での実体）
 * @param self_p mpsx_getopt_t構造体インスタンスポインた
 * @param argc 引き数数
 * @param argv 引数配列ポインタ
			   ...
			   const char *const ostr)
{
	...
	char xtoa_buf[MPSX_XTOA_DEC64_BUFSZ];
	    ...
	    mpsx_i64toadec( (int64_t)argc, xtoa_buf, xtoa_bufsz);
...
/**
 * @fn int mpsx_getopt(mpsx_getopt_t * self_p, int argc, char *argv[], const char *optstring)
 ...
 * @retval -1(RETFINI) 解析の終了を示す
 */
int mpsx_getopt(mpsx_getopt_t *const self_p, const int argc, const char *const argv[],
...
/**
 * @fn static int own_getopt_long_exec(mpsx_getopt_t * const self_p, const enum_getopt_long_type_t long_type, int argc, const char * const argv[], const char * const optstring, mpsx_getopt_long_option_t * const lopts, int *index_p);
 * @brief 単語対応のコマンド引数解析をおこなう（内部処理関数)
 * @param self_p mpsx_getopt_t構造体インスタンスポインタ
				...
				int *const index_p)
{
	...
	IFDBG5THEN {
	    ...
	    mpsx_i64toadec( (int64_t)self_p->optind, xtoa_buf_a, xtoa_bufsz);
	    mpsx_i64toadec( (int64_t)argc, xtoa_buf_b, xtoa_bufsz);
	...
	if (match != -1) {
	    const mpsx_getopt_long_option_t *t = &lopts[match];
	    IFDBG3THEN {
		const size_t xtoa_bufsz=MPSX_XTOA_DEC64_BUFSZ;
		char xtoa_buf[MPSX_XTOA_DEC64_BUFSZ];

		mpsx_i64toadec( (int64_t)match, xtoa_buf, xtoa_bufsz);
		DBMS3(stderr, "match : %s %s"EOL_CRLF,
		    xtoa_buf, t->name);
	    }
	    ++(self_p->optind);
	    switch (t->has_arg) {
	    case MPSX_GETOPT_NO_ARGUMENT:
		self_p->optarg = NULL;
		break;
	    case MPSX_GETOPT_OPTIONAL_ARGUMENT:
	    case MPSX_GETOPT_REQUIRED_ARGUMENT:
		if (has_equal) {
		...
		if (t->has_arg == MPSX_GETOPT_REQUIRED_ARGUMENT) {
...
/**
 * @fn int mpsx_getopt_long(mpsx_getopt_t *self_p, int argc, char *argv[], const char *optstring, mpsx_getopt_long_option_t *lopts, int *index_p)
 ...
 * @retval -1 解析の終了を示す
 */
int mpsx_getopt_long(mpsx_getopt_t *const self_p, const int argc, const char *const argv[],
			const char *const optstring,
			const mpsx_getopt_long_option_t *const lopts, int *const index_p)
{
...
/**
 * @fn int mpsx_getopt_long_only(mpsx_getopt_t *self_p, int argc, char *argv[], const char *optstring, mpsx_getopt_long_option_t *lopts, int *index_p)
 * @brief 単語対応のコマンド引数解析をおこなう
 *	基本的にmpsx_getopt_long()と同じなので、 詳細はmpsx_getopt_longを参照。
 ...
 * @retval -1 解析の終了を示す
 */
int mpsx_getopt_long_only(mpsx_getopt_t *const self_p, const int argc,
