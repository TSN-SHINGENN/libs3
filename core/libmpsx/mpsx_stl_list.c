...
#define get_stl_list_ext(s) MPSX_STATIC_CAST(stl_list_ext_t*,(s)->ext)
#define get_const_stl_list_ext(s) MPSX_STATIC_CAST(const stl_list_ext_t*,(s)->ext)
...
extern int mpsx_stl_list_pop_front(mpsx_stl_list_t *const)
	__attribute__ ((optimize("Os")));
extern int mpsx_stl_list_remove_at(mpsx_stl_list_t *const, const size_t)
	__attribute__ ((optimize("Os")));
extern int mpsx_stl_list_push_back(mpsx_stl_list_t *const, const void *const, const size_t)
	__attribute__ ((optimize("Os")));
extern int mpsx_stl_list_insert(mpsx_stl_list_t *const, const size_t, const void *const, const size_t)
	__attribute__ ((optimize("Os")));
extern int mpsx_stl_list_remove_at(mpsx_stl_list_t *const, const size_t)
	__attribute__ ((optimize("Os")));
extern int mpsx_stl_list_overwrite_element_at( mpsx_stl_list_t *const, const size_t, const void *const, const size_t)
	__attribute__ ((optimize("Os")));
#else /* Normal */
static queitem_t *list_search_item(stl_list_ext_t *const e,
				    const size_t num);
...
/**
 * @fn int mpsx_stl_list_init( mpsx_stl_list_t *const self_p, const size_t sizof_element)
 * @brief 双方向キューオブジェクトを初期化します
 * @param self_p mpsx_stl_list_t構造体インスタンスポインタ
 ...
 */
int mpsx_stl_list_init(mpsx_stl_list_t *const self_p,
			   const size_t sizof_element)
{
    stl_list_ext_t *e = NULL;
    memset(self_p, 0x0, sizeof(mpsx_stl_list_t));

    e = (stl_list_ext_t *)
	mpsx_malloc(sizeof(stl_list_ext_t));
    if (NULL == e) {
	DBMS1(stderr, "mpsx_stl_list_init : mpsx_malloc(ext) fail" EOL_CRLF);
	return EAGAIN;
    }
...
/**
 * @fn int mpsx_stl_list_destroy( mpsx_stl_list_t *const self_p )
...
int mpsx_stl_list_destroy(mpsx_stl_list_t *const self_p)
{
    ...
    result = mpsx_stl_list_clear(self_p);
    if (result) {
	DBMS1(stderr, "mpsx_stl_list_destroy : mpsx_stl_list_clear fail" EOL_CRLF);
	return EBUSY;
    }
...
/**
 * @fn int mpsx_stl_list_push_back(mpsx_stl_list_t *const self_p, const void *const el_p, const int sizof_element)
 * @brief 双方向キューの後方にエレメントを追加します
 * @param self_p mpsx_stl_list_t構造体インスタンスポインタ
 * @param el_p エレメントポインタ
 * @param sizof_element エレメントサイズ(mpsx_stl_list_initで指定した以外のサイズはエラーとします)
 * @retval 0 成功
 * @retval EINVAL エレメントサイズが異なる
 * @retval EAGAIN リソースの獲得に失敗
 * @retval -1 それ以外の致命的な失敗
 */
int mpsx_stl_list_push_back(mpsx_stl_list_t *const self_p,
				...
				const size_t sizof_element)
{
    ...
    DBMS3(stderr, "mpsx_stl_list_push_back : execute" EOL_CRLF);

    if (e->cnt == 0) {
	return mpsx_stl_list_insert(self_p, 0, el_p, sizof_element);
    }
    ...
    f = (queitem_t *) mpsx_malloc(sizeof(queitem_t) + e->sizof_element);
    if (NULL == f) {
	DBMS1(stderr, "mpsx_stl_list_push : mpsx_malloc(queitem_t) fail" EOL_CRLF);
	status = EAGAIN;
	goto out;
    }
...
/**
 * @fn int mpsx_stl_list_push_front(mpsx_stl_list_t *const self_p, const void *const el_p, const size_t sizof_element)
 ...
 * @retval -1 それ以外の致命的な失敗
 */
int mpsx_stl_list_push_front(mpsx_stl_list_t *const self_p,
				 const void *const el_p,
				 const size_t sizof_element)
{
    return mpsx_stl_list_insert(self_p, 0, el_p, sizof_element);
...
/**
 * @fn int mpsx_stl_list_pop_front(mpsx_stl_list_t *self_p)
...
/**
 * @fn int mpsx_stl_list_pop_back(mpsx_stl_list_t *const self_p)
...
/**
 * @fn int mpsx_stl_list_clear( mpsx_stl_list_t *const self_p )
...
int mpsx_stl_list_clear(mpsx_stl_list_t *const self_p)
{
    ...
    for (i = e->cnt; i != 0; --i) {
	result = mpsx_stl_list_pop_front(self_p);
	if (result) {
	    IFDBG1THEN {
		const size_t xtoa_bufsz=MPSX_XTOA_DEC64_BUFSZ;
		char xtoa_buf[MPSX_XTOA_DEC64_BUFSZ];

		mpsx_i64toadec( (int64_t)i, xtoa_buf, xtoa_bufsz);
		DBMS1(stderr,
		  "mpsx_stl_list_clear : mpsx_stl_list_pop_front[%s] fail" EOL_CRLF, xtoa_buf);
	    }
...
/**
 * @fn int mpsx_stl_list_get_pool_cnt( mpsx_stl_list_t *const self_p );
...
/**
 * @fn int mpsx_stl_list_is_empty( mpsx_stl_list_t *self_p)
...
/**
 * @fn int mpsx_stl_list_get_element_at( mpsx_stl_list_t *const self_p, const int num, void *const el_p, const size_t sizof_element);
 ...
 */
int mpsx_stl_list_get_element_at(mpsx_stl_list_t *const self_p,
				     ...
				     const size_t sizof_element)
{
    ...
    if (mpsx_stl_list_is_empty(self_p)) {
	return EACCES;
    }
    ...
    mpsx_memcpy(el_p, item_p->data, e->sizof_element);

    return 0;
}

/**
 * @fn int mpsx_stl_list_remove_at(mpsx_stl_list_t *const self_p, const size_t num)
 ...
 * @retval EINVAL sizof_elementのサイズが異なる(小さい）
 */
int mpsx_stl_list_remove_at(mpsx_stl_list_t *const self_p,
			       const size_t num)
{
    stl_list_ext_t *const e = get_stl_list_ext(self_p);
...
/**
 * @fn void mpsx_stl_list_dump_element_region_list( mpsx_stl_list_t *const self_p)
 * @brief 双方向キューに保存されている要素の情報をダンプします
 *	主にこのデバッグ用
 * @param self_p mpsx_stl_list_t構造体インスタンスポインタ
 **/
void mpsx_stl_list_dump_element_region_list(mpsx_stl_list_t *const
						self_p)
{
    const stl_list_ext_t *const e = get_const_stl_list_ext(self_p);
...
__attribute__ ((unused))
    ...
    DMSG(stdout,
	    "mpsx_stl_list_dump_element_region_list : execute\n");
    mpsx_u64toadec( (uint64_t)e->sizof_element, xtoa_buf, xtoa_bufsz);
    DMSG(stderr, "sizof_element = %s" EOL_CRLF, xtoa_buf);
    mpsx_u64toadec( (uint64_t)e->cnt, xtoa_buf, xtoa_bufsz);
    DMSG(stderr, "numof_elements = %s" EOL_CRLF, xtoa_buf);
    mpsx_i64toadec( (int64_t)e->start_id, xtoa_buf, xtoa_bufsz);
...
/**
 * @fn int mpsx_stl_list_insert( mpsx_stl_list_t *const self_p, const size_t num, const void *const el_p, const size_t sizof_element )
 * @brief 双方向キューの指定されたエレメント番号の直前に、データを挿入します.
 *	numに0を指定した場合は、mpsx_listue_push_front()と同等の動作をします。
 * @param self_p mpsx_stl_list_t構造体インスタンスポインタ
 * @param num 0から始まるキュー先頭からのエレメント配列番号
 * @param el_p エレメントデータポインタ
 * @param sizof_element エレメントサイズ(主にエレメントサイズ検証向け)
 ...
 * @retval EAGAIN リソースの獲得に失敗
 */
int mpsx_stl_list_insert(mpsx_stl_list_t *const self_p,
			     ...
			     const size_t sizof_element)
{
	...
	mpsx_u64toadec( (uint64_t)num, xtoa_buf_a, xtoa_bufsz);
	mpsx_u64toadec( (uint64_t)e->cnt, xtoa_buf_b, xtoa_bufsz);
	DBMS3(stderr, "mpsx_stl_list_insert : execute" EOL_CRLF);
	DBMS3(stderr, "mpsx_stl_list_insert : num=%s e->cnt=%s" EOL_CRLF,
	...
	IFDBG3THEN {
	    ...
	    DBMS3(stderr,
	      "mpsx_stl_list_insert : e->sizof_element=%s sizof_element=%s" EOL_CRLF, xtoa_buf_a, xtoa_buf_b);
	}
	return EINVAL;
    }
    ...
    DBMS3(stderr, "mpsx_stl_list_insert : front=0x%p e->base=0x%p" EOL_CRLF,
	  front, &e->base);

    f = (queitem_t *) mpsx_malloc(sizeof(queitem_t) + e->sizof_element);
    if (NULL == f) {
	DBMS1(stderr, "mpsx_stl_list_push : mpsx_malloc(queitem_t+element) fail" EOL_CRLF);
	status = EAGAIN;
	goto out;
    }
...
/**
 * @fn int mpsx_stl_list_overwrite_element_at( mpsx_stl_list_t *const self_p,  const size_t num, const void *const el_p, const size_t sizof_element)
...
int mpsx_stl_list_overwrite_element_at( mpsx_stl_list_t *const self_p,  const size_t num, const void *const el_p, const size_t sizof_element)
{
    ...
    mpsx_memcpy(item_p->data, el_p, e->sizof_element);

    return 0;
}

/**
 * @fn int mpsx_stl_list_front( mpsx_stl_list_t *const self_p, const void *const el_p, const size_t sizeof_element)
...
/**
 * @fn int mpsx_stl_list_back( mpsx_stl_list_t *const self_p, void *const el_p, const size_t sizof_element)
