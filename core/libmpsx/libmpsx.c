/*! \mainpage libmpsxリファレンスガイド
 *
 * \section 概要
 * libmpsxはLinux/MACOSX/Windows/Solaris/QNX/AdobeActionScript等の異なるOSで提供されるAPIの仕様を吸収するための静的ライブラリです。<br>
 * アプリケーション作成時、異なる開発環境やOSでも一つのソースコードで実行バイナリを作成することを支援します。<br>
 * 以下の利点を考慮しC言語で書かれています。<br>
 * &nbsp; ・多くの関数をネイティブ近い形でサポートしています。<br>
 * &nbsp; ・個性のあるライブラリはlibmpsx内部での使用を避けています。よって、JAVAや.net等の他言語との接続を容易に行えます。<br>
 * &nbsp; &nbsp; ※ libmpsxの関数を他言語から直接呼び出す場合は必要な環境を用意してください。<br>
 * 
 * \section 背景
 ...
 * &nbsp; &nbsp; libmpsxをリンクする際に次のリンクオプション( -lm -lsocket)を追加してください。<br>
 ...
 * &nbsp; &nbsp; MACOSXアプリケーションにlibmpsxをリンクする際には、次のリンクオプション( -lm -lpthread -framework Carbon)を追加してください。<br>
 ...
 * &nbsp;   バーチャルマシンでの実行となるため SSE命令等は使えません。libmpsxビルド時に対応する関数は排除されます<br>
 * &nbsp; &nbsp; libmpsxをリンクする際に次のリンクオプション( -lm -lpthread)を追加してください。<br>
 * &nbsp; ・FreeRTOS
 * &nbsp;   組み込みリアルタイム環境の時流に従い、検討していきます。
 * &nbsp; ・Xilinx Standaloneカーネル<br>
 * &nbsp;   上記カーネルではlibmpsxのかなりのPOSIX互換関数が使えないように制限されます。<br>
 ...
 * &nbso;   しかし、libmpsxはstatic領域を使って簡易的なメモリ領域を管理するためのアロケーターを用意しています。<br>
 * &nbso;   mpsx_lite_mallocater APIを使用して、ヒープ領域を使わずに処理することが可能です。<br>
 * &nbso;   mpsxライブラリの関数を使用するには mpsx_lite_mallocater_init()を使用してメモリを割り当ててください。<br>
 * &nbsp; 　makefile中の定義で、OS標準のメモリ管理機構に切り替えることも可能です。<br>
 * &nbsp; 　<br>
 * &nbsp; 　libmpsxプロジェクト使用する場合は Makefile Project with Existing Codeでソースコードを追加するフォルダを作成します。<br>
 * &nbsp; 　libmpsxソースコードを先のフォルダにコピーしてください。<br>
 * &nbsp; 　よってlibmpsxソースコードはバージョン管理機構から分岐し,てPlatformSDKに併合して管理します<br>
 ...
 * &nbsp;   Debug   : "all" -> "debug"<br>
 * &nbsp;   libmpsxはビルド時に-ffunction-sections -fdata-sectionsオプションを用いて、リンク時に不要な関数をそぎ落とす前処理がされています。<br>
 ...
 * &nbsp; &nbsp; &nbsp; ARMにもSIMD命令はあるようですがlibmpsxに実装しないといけません。 <br>
 * &nbsp; &nbsp; ※ Sparc系については SunOS系の対応の開発休止をVW部で決めたので現在未対応です。 <br>
 *
 * \section 注意事項
 *
 * libmpsxは社内でアプリケーション及びライブラリを開発するために使用されます。<br>
 * 単体での社外公開はしていません。<br>
 ...
 * <br>
 * &nbsp; &nbsp; mpsx_getopt()の内部実装であるown_getopt_exec()はAT&T社パブリックドメインライセンスとなっているgetopt()の拡張版となっています。<br>
 * <br>
 * \section 利用について
...
/**
 * @file libmpsx.c
 * @brief libmpsxライブラリの情報取得関数
 */

/* this */
#include "mpsx_endian.h"
#include "libmpsx_revision.h"
#include "libmpsx.h"

/**
 * @fn const char *const mpsx_get_lib_revision(void)
 * @brief libmpsxの版管理番号の取得
 * @return 版管理番号文字列ポインタ
 */
const char *mpsx_get_lib_revision(void)
{
    return LIB_MPSX_REVISION;
}

/**
 * @fn const char *const mpsx_get_lib_name(void)
 * @brief libmpsxライブラリの標準名を返します
 * @return 標準名文字列ポインタ
 */
const char *mpsx_get_lib_name(void)
{
    return LIB_MPSX_NAME;
}

const char *const _libmpsx_idkeyword =
    "@(#)libmpxx_idkeyword : " LIB_MPSX_NAME " revision"
    LIB_MPSX_REVISION
    " CopyRight (C) TSN･SHINGENN All Rights Reserved.";

/**
 * @fn const char *mpsx_get_idkeyword(void)
 * @brief libmpsx_idkeywordを返します。
 * @return idkeyword文字列ポインタ
 */
const char *mpsx_get_idkeyword(void)
{
    return _libmpsx_idkeyword;
}
