...
/**
 * @fn void _mpxx_vprintf(void (*_putc_cb)(int), const char *fmt, va_list ap)
 * @brief
 *
 **/
void _mpxx_vprintf(void (*_putc_cb)(int), const char *fmt, va_list ap)
{
...
//	    va_end(ap);
	...
	for( opts.flags = 0; (NULL != mpxx_strchr("'-+ #0", *fmt)); ++fmt) {
	    switch (*fmt) {
	    case '\'': {
...
//		flags |= WITH_SIGN_CHAR;
	...
	if(*fmt == '*') {
	    ...
	    for(;mpxx_ascii_isdigit(*fmt); ++fmt) {
		length *= 10;
		length += mpxx_ctoi(*fmt);
	    }
	}
		...
		while (mpxx_ascii_isdigit(*fmt) ) {
		    precision *= 10;
		    precision += mpxx_ctoi(*fmt);
		    ++fmt;
		}
	    }
	}

	while (mpxx_strchr("hljzt", *fmt)) {
	    switch (*fmt++) {
            case 'h': {
		int_type = MPXX_STATIC_CAST( enum_integer_type_t, int_type-1);
		} break;
	    case 'l': {
		int_type = MPXX_STATIC_CAST( enum_integer_type_t, int_type-1);
		} break;
...
/**
 * @fn int mpxx_snprintf( char * __restrict buf, size_t size, const char * __restrict fmt, ...)
 ...
 */
int mpxx_snprintf(char *__restrict buf, size_t size,
		     const char *__restrict fmt, ...)
{
    ...
    va_start(arg, fmt);
    retval = mpxx_vsnprintf(buf, size, fmt, arg);
    va_end(arg);
...
/**
 * @fn int mpxx_vsnprintf( char * __restrict buf, size_t size, const char * __restrict fmt, va_list ap)
 ...
 *	ERANGE bufにfmt指定された文字列が収まらなかった
 */
int mpxx_vsnprintf(char *__restrict buf, size_t size,
...
#else
#error not implemented mpxx_vsnprintf()
#endif
}
