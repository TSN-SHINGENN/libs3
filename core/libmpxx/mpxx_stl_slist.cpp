...
#define get_stl_slist_ext(s) MPXX_STATIC_CAST(stl_slist_ext_t*,(s)->ext)
#define get_const_stl_slist_ext(s) MPXX_STATIC_CAST(const stl_slist_ext_t*,(s)->ext)
...
extern int mpxx_stl_slist_init( mpxx_stl_slist_t *const self_p, const size_t sizof_element)
	__attribute__ ((optimize("Os")));
extern int mpxx_stl_slist_destroy( mpxx_stl_slist_t *const self_p)
	__attribute__ ((optimize("Os")));
extern int mpxx_stl_slist_push( mpxx_stl_slist_t *const self_p, const void *const el_p, const size_t sizof_element)
	__attribute__ ((optimize("Os")));
extern int mpxx_stl_slist_pop( mpxx_stl_slist_t *const self_p)
	__attribute__ ((optimize("Os")));
extern int mpxx_stl_slist_front( mpxx_stl_slist_t *const self_p, void *const el_p, const size_t  sizof_element )
	__attribute__ ((optimize("Os")));
extern int mpxx_stl_slist_get_element_at( mpxx_stl_slist_t *const self_p, const size_t num, void *const el_p, const size_t sizof_element)
	__attribute__ ((optimize("Os")));
extern int mpxx_stl_slist_clear( mpxx_stl_slist_t *const self_p)
	__attribute__ ((optimize("Os")));
extern size_t mpxx_stl_slist_get_pool_cnt( mpxx_stl_slist_t *const self_p)
	__attribute__ ((optimize("Os")));
extern int mpxx_stl_slist_is_empty( mpxx_stl_slist_t *const self_p)
	__attribute__ ((optimize("Os")));

extern int mpxx_stl_slist_back( mpxx_stl_slist_t *const self_p, void *const el_p, const size_t  sizof_element )
	__attribute__ ((optimize("Os")));
extern int mpxx_stl_slist_insert_at( mpxx_stl_slist_t *const self_p, const size_t no, void *const el_p, const size_t  sizof_element )
	__attribute__ ((optimize("Os")));
...
/**
 * @fn mpxx_stl_slist_t *mpxx_stl_slist_init( mpxx_stl_slist_t *const self_p, const size_t sizof_element)
 * @brief キューオブジェクトを初期化します
 * @param self_p mpxx_stl_slist_t構造体インスタンスポインタ
 * @param sizof_element エレメントのサイズ
 * @retval 0 成功
 * @retval EAGAIN リソースの獲得に失敗
 * @retval -1 それ以外の致命的な失敗
 */
int mpxx_stl_slist_init(mpxx_stl_slist_t *const self_p,
			   const size_t sizof_element)
{
    stl_slist_ext_t *e = NULL;
    memset(self_p, 0x0, sizeof(mpxx_stl_slist_t));

    e = (stl_slist_ext_t *)
	mpxx_malloc(sizeof(stl_slist_ext_t));
    if (NULL == e) {
	DBMS1(stderr, "mpxx_stl_slist_init : mpxx_malloc(ext) fail" EOL_CRLF);
	return EAGAIN;
    }
...
/**
 * @fn int mpxx_stl_slist_destroy( mpxx_stl_slist_t *const self_p )
...
int mpxx_stl_slist_destroy(mpxx_stl_slist_t *const self_p)
{
    ...
    result = mpxx_stl_slist_clear(self_p);
    if (result) {
	DBMS1(stderr, "mpxx_stl_slist_destroy : que_clear fail" EOL_CRLF);
	return EBUSY;
    }
...
/**
 * @fn int mpxx_stl_slist_push(mpxx_stl_slist_t *const self_p, const void *const el_p, const size_t sizof_element)
 * @brief キューにエレメントを追加します
 * @param self_p mpxx_stl_slist_t構造体インスタンスポインタ
 * @param el_p エレメントポインタ
 * @param sizof_element エレメントサイズ(mpxx_stl_slist_initで指定した以外のサイズはエラーとします)
 * @retval 0 成功
 * @retval EINVAL エレメントサイズが異なる
 * @retval EAGAIN リソースの獲得に失敗
 * @retval -1 それ以外の致命的な失敗
 */
int mpxx_stl_slist_push(mpxx_stl_slist_t *const self_p, const void *const el_p,
			   const size_t sizof_element)
{
    int status;
    stl_slist_ext_t * const e = get_stl_slist_ext(self_p);
    ...
    f = (fifoitem_t *) mpxx_malloc(sizeof(fifoitem_t) + e->sizof_element);
    if (NULL == f) {
	DBMS1(stderr,
	      "mpxx_stl_slist_push : mpxx_malloc(fifoitem_t) fail" EOL_CRLF);
	status = EAGAIN;
	goto out;
    }
...
/**
 * @fn int mpxx_stl_slist_pop(mpxx_stl_slist_t *const self_p)
...
/**
 * @fn int mpxx_stl_slist_front( mpxx_stl_slist_t *const self_p, void *const el_p, const size_t  sizof_element )
 ...
 * @retval 0 成功
 */
int mpxx_stl_slist_front(mpxx_stl_slist_t *const self_p, void *const el_p,
			    const size_t sizof_element)
{
    stl_slist_ext_t * const e = get_stl_slist_ext(self_p);
    ...
    mpxx_memcpy(el_p, f->data, e->sizof_element);

    return 0;
}

/**
 * @fn int mpxx_stl_slist_clear( mpxx_stl_slist_t *const self_p )
...
int mpxx_stl_slist_clear(mpxx_stl_slist_t *const self_p)
{
    ...
    for (n = e->cnt; n != 0; --n) {
	result = mpxx_stl_slist_pop(self_p);
	if (result) {
	    IFDBG1THEN {
		...
		DBMS1(stderr,
		  "mpxx_stl_slist_clear : mpxx_stl_slist_pop[%s] fail" EOL_CRLF, xtoa_buf);
	    }
	    return -1;
	}
    }
...
/**
 * @fn int mpxx_stl_slist_get_pool_cnt( mpxx_stl_slist_t *const self_p );
...
/**
 * @fn int mpxx_stl_slist_is_empty( mpxx_stl_slist_t *const self_p)
...
/**
 * @fn int mpxx_stl_slist_get_element_at( mpxx_stl_slist_t *const self_p, const size_t num, void *const el_p, const int sizof_element);
 ...
 */
int mpxx_stl_slist_get_element_at(mpxx_stl_slist_t *const self_p,
				       ...
				       const size_t sizof_element)
{
    ...
    if (mpxx_stl_slist_is_empty(self_p)) {
	return EACCES;
    }
...
/**
 * @fn int mpxx_stl_slist_back( mpxx_stl_slist_t *const self_p, void *const el_p, const size_t  sizof_element )
...
/**
 * @fn int mpxx_stl_slist_insert_at( mpxx_stl_slist_t *const self_p, const size_t no, void *const el_p, const size_t  sizof_element )
...
int mpxx_stl_slist_insert_at( mpxx_stl_slist_t *const self_p, const size_t no, void *const el_p, const size_t  sizof_element )
{
    ...
    if (NULL == f) {
	DBMS1(stderr,
	      "mpxx_stl_slist_insert_at : mpxx_malloc(fifoitem_t) fail" EOL_CRLF);
	return EAGAIN;
    }
...
/**
 * @fn int mpxx_stl_slist_erase_at( mpxx_stl_slist *const self_p, const size_t no)
