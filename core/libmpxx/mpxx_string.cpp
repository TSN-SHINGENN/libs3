...
extern void *mpxx_memcpy(void *const dest, const void *const src, const size_t n)
		__attribute__ ((optimize("Os")));
extern void *mpxx_memcpy_path_through_level_cache(void *const dst, const void *const src, const size_t n)
		__attribute__ ((optimize("Os")));
extern void *mpxx_memset_path_through_level_cache(void *const d, const int c, const size_t n)
		__attribute__ ((optimize("Os")));

extern int mpxx_strcasecmp(const char *const s1, const char *const s2)
		__attribute__ ((optimize("Os")));
extern int mpxx_strncasecmp(const char *const s1, const char *const s2, const size_t n)
		__attribute__ ((optimize("Os")));
extern int mpxx_strtoupper(const char *const before_str, char *const after_buf, const size_t after_bufsize)
		__attribute__ ((optimize("Os")));
extern int mpxx_strtolower(const char *const before_str, char *const after_buf, const size_t after_bufsize)
		__attribute__ ((optimize("Os")));
extern char *mpxx_strcasestr(const char *const haystack, const char *const needle)
		__attribute__ ((optimize("Os")));
extern char *mpxx_strtrim(char *const buf, const char *const accepts, const int c)
		__attribute__ ((optimize("Os")));
...
static void *o32_memcpy( void *dest, const void *src, const size_t n)
{
	...
	size_t b = n;
	uint8_t *d = MPXX_STATIC_CAST(uint8_t*, dest);
	const uint8_t *s = MPXX_STATIC_CAST(const uint8_t*, src);
...
/**
 * @fn void *mpxx_memcpy(void *const dest, const void *const src, const size_t n)
...
{
#if defined(_LIBS3_X86SIMD_ENABLE)
    return mpxx_simd_memcpy(dest, src, n);
...
/**
 * @fn void *mpxx_memcpy_path_through_level_cache(void *const dst, const void *const src, const size_t n)
 ...
 * @retval destポインタ 成功
 */
void *mpxx_memcpy_path_through_level_cache(void *const dst, const void *const src,
					      const size_t n)
{
#if defined(_LIBS3_X86SIMD_ENABLE)
    return mpxx_simd_memcpy_path_through_level_cache(dst, src, n);
#else
...
/**
 * @fn void *mpxx_memset_path_through_level_cache(void *const d, const int c, const size_t n)
 ...
 * @retval d値 成功
 */
void *mpxx_memset_path_through_level_cache(void *const d, const int c,
					      const size_t n)
{
#if defined(_LIBS3_X86SIMD_ENABLE)
    return mpxx_simd_memset_path_through_level_cache(d, c, n);
#else
    return memset(d, c, n);
#endif
...
/**
 * @fn int mpxx_strcasecmp(const char *const s1, const char *const s2)
...
#else
#error 'can not define mpxx_strncasecmp()'
#endif
}


/**
 * @fn int mpxx_strncasecmp(const char *const s1, const char *const s2, const size_t n)
 * @brief 二つの文字列 s1 と s2 を、 大文字小文字を区別せずに比較します。
...
int mpxx_strncasecmp(const char *const s1, const char *const s2, const size_t n)
{
    ...
    ref1 = mpxx_malloca(ref1_len);
    memset(ref1, 0x0, ref1_len);
    ref2 = mpxx_malloca(ref2_len);
    memset(ref2, 0x0, ref1_len);

    result = mpxx_strtolower(s1, ref1, ref1_len);
    if (result) {
	status = -1;
	goto out;
    }
    result = mpxx_strtolower(s2, ref2, ref2_len);
    if (result) {
	status = -1;
	goto out;
    }
...
/**
 * @fn int mpxx_strtoupper(const char *const before_str, char *const after_buf, const size_t after_bufsize)
 ...
 * @retval 0 成功
 */
int mpxx_strtoupper(const char *const before_str, char *const after_buf,
...
/**
 * @fn int mpxx_strtolower(const char *const before_str, char *const after_buf, const size_t after_bufsize)
 ...
 * @retval 0 成功
 */
int mpxx_strtolower(const char *const before_str, char *const after_buf,
		       const size_t after_bufsize)
{
    const size_t before_len = strlen(before_str);
    size_t i;
	...
	mpxx_i64toadec( (int64_t)before_len, xtoa_buf, xtoa_bufsz);
...
/**
 * @fn char *mpxx_strcasestr(const char *const haystack, const char *const needle)
...
char *mpxx_strcasestr(const char *const haystack, const char *const needle)
{
    ...
    datbuf = (char *) mpxx_malloc(datbuf_len);
    if (NULL == datbuf) {
	errno = ENOMEM;
	return NULL;
    }
    target = (char *) mpxx_malloc(target_len);
    if (NULL == target) {
	errno = ENOMEM;
	return NULL;
    }
    result = mpxx_strtolower(haystack, datbuf, datbuf_len);
    if (result) {
	retp = NULL;
	goto out;
    }
    result = mpxx_strtolower(needle, target, target_len);
    if (result) {
	retp = NULL;
	goto out;
    }
...
#else
#error 'can not define mpxx_strcasestr()'
#endif
}

/**
 * @fn char *mpxx_strtrim(char *const buf, const char *const accepts, const int c)
...
/**
 * @fn char *mpxx_strchr(const char *const str, const int code)
 * @brief 文字列 str 中に最初にASCII文字code が現れた位置へのポインターを返す。 
 * @param s ASCII文字列ポインタ
 * @param c 検索コード
 * @retval NULL cで指定された文字が見つからない
...
/**
 * @fn char *mpxx_strrchr(const char *const str, const int code)
