...
/**
 * @fn static int clock_realtime(mpxx_timespec_t * ts_p)
 * @brief 高精度実時間の取得起原は 1970/01/01
 * @param mts_p mpxx_mtimespec_t構造体ポインタ
 * @retval 0 成功
 * @retval EINVAL : 処理できない
 * @retval EFAULT : mts_pポインタがアクセス不可能
 * @retval ENOTSUP : サポートされていない
 */
static int clock_realtime(mpxx_timespec_t * const ts_p)
{
    ...
    return ENOTSUP;
#else				/* others */
#error not impliment clock_realtime() in mpxx_time.c
#endif				/* others */
...
/**
 * @fn static int clock_realtime(mpxx_timespec_t * const ts_p)
 ...
 * @retval EFAULT : mts_pポインタがアクセス不可能
 */
static int clock_monotinoc(mpxx_timespec_t * const ts_p)
{
	...
	mpxx_i64toadec( (int64_t)counts, xtoa_buf, xtoa_bufsz);
	DBMS3(stderr, "clock_monotinoc counts=%s" EOL_CRLF, xtoa_buf);
	mpxx_i64toadec( (int64_t)freq, xtoa_buf, xtoa_bufsz);
...
#define TICKS_TO_MS(x) ((x) * SYS_MSPERTICK)
    ...
    return ENOTSUP;
#else				/* others */
#error not impliment clock_monotinoc() in mpxx_time.c
#endif
...
/**
 * @fn int mpxx_clock_getmtime( mpxx_clockid_t id, mpxx_mtimespec_t *mts_p)
 * @brief POSIX clock_gettime()のマルチOS版, 精度はちょっと劣る
 * @param id mpxx_clockid_t値
 * @param mts_p mpxx_mtimespec_t構造体ポインタ
 *	sec : 秒
 *	msec: ミリ秒
 * @retval 0 成功
 ...
 * @retval EFAULT : mts_pポインタがアクセス不可能
 */
int mpxx_clock_getmtime(const mpxx_clockid_t id,
			   mpxx_mtimespec_t * const mts_p)
{
    int status;
    mpxx_timespec_t ts;

    DBMS5(stderr, "mpxx_clock_getmtime : execute" EOL_CRLF);
    ...
    switch (id) {
    case MPXX_CLOCK_REALTIME:
	status = clock_realtime(&ts);
	if (status) {
	    goto out;
	}
	break;
    case MPXX_CLOCK_MONOTONIC:
 ...
 * @retval EFAULT : mts_pポインタがアクセス不可能
 */
int mpxx_clock_gettime(const mpxx_clockid_t id,
			  mpxx_timespec_t * const mts_p)
{
    int status;
...
/**
 * @fn struct tm *mpxx_gmtime_r( const time_t * const time_p, struct tm * const rettm_p)
 ...
 */
struct tm *mpxx_gmtime_r(const time_t * const time_p,
...
#else
#error not impliment mpxx_gmtime_r()
#endif
}
