...
/**
 * @fn int mpxx_ascii_isprint(const int c)
...
#else
#error 'not impliment mpxx_ascii_isprint() on target OS'
#endif
}

/**
 * @fn int mpxx_ascii_isspace(const int c)
...
#else
#error 'not impliment mpxx_ascii_isspace() on target OS'
#endif
}

/**
 * @fn int mpxx_ascii_isdigit(const int c)
...
int mpxx_ascii_isdigit(const int c)
{
    ...
    return ((c <= '9') && (c >= '0'));
#else
#error 'not impliment mpxx_ascii_isdigit() on target OS'
#endif
}

/**
 * @fn int mpxx_ascii_isalpha(const int c)
...
#else
#error 'not impliment mpxx_ascii_isalpha() on target OS'
#endif
}

/**
 * @fn int mpxx_ascii_toupper(const int c)
...
#else
#error 'not impliment mpxx_ascii_toupper() on target OS'
#endif
}

/**
 * @fn int mpxx_ascii_tolower(const int c)
 * @brief　入力コード番号をASCIIコードに照らしてアルファベットを大文字に変換する
 *	変換できない場合は 引数c の値をそのまま戻す
 * return 条件にあったアスキーコード
 **/
int mpxx_ascii_tolower(const int c)
{
    ...
    return (c <= 'Z' && c >= 'A' ? c + 32 : c); 
#else
#error 'not impliment mpxx_ascii_tolower() on target OS'
#endif
}

/**
 * @fn int mpxx_ascii_islower(const int c)
...
int mpxx_ascii_islower(const int c)
{
    ...
    return (c <= 'z' && c >= 'a' ? c : 0); 
#else
#error 'not impliment mpxx_ascii_islower() on target OS'
#endif
}

/**
 * @fn int mpxx_ascii_isupper(const int c)
 * @brief 入力コード番号をASCIIコードに照らして小文字アルファベットであるかどうかを確認する
 * retval 0 不適合
 * retval 0以外 小文字アルファベットの場合，cの値を戻す
 **/
int mpxx_ascii_isupper(const int c)
{
    ...
    return (c <= 'Z' && c >= 'A' ? c : 0); 
#else
#error 'not impliment mpxx_ascii_isupper() on target OS'
#endif
}
