...
mpxx_lite_mallocater_t _mpxx_lite_mallocater_heap_emulate_obj;


#if defined(__GNUC__) || defined(_MPXX_MICROCONTROLLER_ENABLE_GCC_OPTIMIZE_FOR_SIZE)
extern void *mpxx_lite_mallocater_alloc_with_obj(mpxx_lite_mallocater_t *const, const size_t)
	__attribute__ ((optimize("O2")));
extern void mpxx_lite_mallocater_free_with_obj(mpxx_lite_mallocater_t *const, void * const)
	__attribute__ ((optimize("O2")));
#endif /* end of Optimize option for GNU C Compiler */

#if defined(_MPXX_MICROCONTROLLER_ENABLE_GCC_OPTIMIZE_FOR_SIZE)
extern void *mpxx_lite_mallocater_alloc(const size_t size)
	__attribute__ ((optimize("Os")));
extern void mpxx_lite_mallocater_free(void * const ptr)
	__attribute__ ((optimize("Os")));
extern void *mpxx_lite_mallocater_realloc(void * const ptr, const size_t size)
	__attribute__ ((optimize("Os")));
extern int mpxx_lite_mallocater_init_obj(mpxx_lite_mallocater_t *const self_p, void *const buf, const size_t bufsize)
	__attribute__ ((optimize("Os")));
extern void *mpxx_lite_mallocater_alloc_with_obj(mpxx_lite_mallocater_t *const self_p,const size_t size)
	__attribute__ ((optimize("Os")));
extern void mpxx_lite_mallocater_free_with_obj(mpxx_lite_mallocater_t *const self_p, void * const ptr)
	__attribute__ ((optimize("Os")));
extern void *mpxx_lite_mallocater_realloc_with_obj(mpxx_lite_mallocater_t *const self_p, void *const ptr, const size_t size)
	__attribute__ ((optimize("Os")));
extern int64_t mpxx_lite_mallocater_avphys_with_obj(mpxx_lite_mallocater_t *const self_p)
	__attribute__ ((optimize("Os")));
...
#define get_lite_allocater_own() &(_mpxx_lite_mallocater_heap_emulate_obj)

#define SIZEOF_ALLOCATEHEADER (sizeof(mpxx_lite_malllocate_header_t))
...
#define GET_PTR2HEAD(p) ((mpxx_lite_malllocate_header_t*)((uintptr_t)(p) - SIZEOF_ALLOCATEHEADER))
...
#ifdef __GNUC__
static int region_pointer_check(mpxx_lite_mallocater_t *const, const mpxx_lite_malllocate_header_t * const)
	__attribute__ ((optimize("O2")));
#else
static int region_pointer_check(mpxx_lite_mallocater_t *const, const mpxx_lite_malllocate_header_t * const);
#endif
...
/**
 * @fn int mpxx_lite_mallocater_init_obj(mpxx_lite_mallocater_t *const self_p, void * const buf, const size_t bufsiz )
 ...
 * @retval 0 成功
 **/
int mpxx_lite_mallocater_init_obj(mpxx_lite_mallocater_t *const self_p, void * const buf, const size_t bufsiz )
{
    int result;
    mpxx_lite_mallocater_t * const o = self_p;
    mpxx_lite_malllocate_header_t *h;
    const uint8_t align = (uintptr_t)buf & (ALLOCATER_ALIGN -1);

    DBMS5( stderr, "mpxx_lite_mallocater_init_obj : execute buf=0x" FMT_ZLLX " siz=" FMT_LLU EOL_CRLF, 8, (uint64_t)((uintptr_t)buf), (uint64_t)bufsiz);

    if( bufsiz < (TOTALAREASIZE(1) * 3)) {
	return ENOMEM;
    }

    memset(  o, 0x0, sizeof(mpxx_lite_malllocate_header_t));
	...
	(mpxx_lite_malllocate_header_t*)o->buf;
    ...
    *(uintptr_t*)GET_FOOTER_PTR(h) = (uintptr_t)h;
    ...
    IFDBG5THEN {
	const size_t u2abufsz = MPXX_XTOA_HEX64_BUFSZ;
	char u2abuf_a[MPXX_XTOA_HEX64_BUFSZ];
	char u2abuf_b[MPXX_XTOA_HEX64_BUFSZ];
	char u2abuf_c[MPXX_XTOA_DEC64_BUFSZ];
	char u2abuf_d[MPXX_XTOA_HEX64_BUFSZ];

	    mpxx_u64toa((uintptr_t)self_p, u2abuf_a,u2abufsz,
		      MPXX_XTOA_RADIX_HEX, NULL);
	    mpxx_u64toa((uintptr_t)o->buf, u2abuf_b,u2abufsz,
		      MPXX_XTOA_RADIX_HEX, NULL);
	    mpxx_u64toa(o->bufsiz, u2abuf_c,MPXX_XTOA_DEC64_BUFSZ,
		      MPXX_XTOA_RADIX_DEC, NULL);
	    mpxx_u64toa((uintptr_t)&(self_p->base), u2abuf_d,u2abufsz,
		      MPXX_XTOA_RADIX_HEX, NULL);
	...
	// _mpxx_lite_mallocater_dump_region_list(self_p);
	 abort();
    }
...
/**
 * @fn int mpxx_lite_mallocater_destroy(mpxx_lite_mallocater_t *const self_p)
 * @brief メモリアロケータオブジェクトインスタンスを破棄します。
 *	メモリマッピングが後続の処理に影響しないように管理エリアを0クリアします
 * @param self_p オブジェクトインスタンスポインタ
 */
int mpxx_lite_mallocater_destroy(mpxx_lite_mallocater_t *const self_p)
{
    mpxx_lite_mallocater_t * const o = self_p;
...
/**
 * @fn void *mpxx_lite_mallocater_alloc_with_obj(mpxx_lite_mallocater_t *const self_p, const size_t sz)
 ...
 **/
void *mpxx_lite_mallocater_alloc_with_obj(mpxx_lite_mallocater_t *const self_p, const size_t sz)
{
    mpxx_lite_malllocate_header_t *p;
    int result;
    const size_t totalsz = TOTALAREASIZE(sz); // Ceilling
    void *retptr = NULL;

    DBMS5( stderr, "mpxx_lite_mallocater_alloc_with_obj : execute" EOL_CRLF);

    if(!self_p->init.f.initialized ) {
	errno = EPERM;
	return NULL;
    } else if ( totalsz == 0 ) {
	// DMSG(stderr,"mpxx_lite_mallocater_alloc_with_obj : totalsz=0:sz=%d" EOL_CRLF, sz);
	errno = EINVAL;
	return NULL;
    }

    IFDBG3THEN {
	const size_t u2abufsz = MPXX_XTOA_DEC64_BUFSZ;
	char u2abuf_a[MPXX_XTOA_DEC64_BUFSZ];
	char u2abuf_b[MPXX_XTOA_DEC64_BUFSZ];
	char u2abuf_c[MPXX_XTOA_DEC64_BUFSZ];

	mpxx_u32toa(SIZEOF_ALLOCATEHEADER, u2abuf_a,u2abufsz,
	  MPXX_XTOA_RADIX_DEC, NULL);
	mpxx_u64toa(sz, u2abuf_b,u2abufsz,
	  MPXX_XTOA_RADIX_DEC, NULL);
	mpxx_u64toa(totalsz, u2abuf_c,u2abufsz,
	  MPXX_XTOA_RADIX_DEC, NULL);
	...
	if( AREA_IS_FREE(p) && (p->size > totalsz)) {
		...
		*(uintptr_t*)GET_FOOTER_PTR(p) = (uintptr_t)p;
		...
		/* 後半を空き領域にする */
		mpxx_lite_malllocate_header_t * const s =
			(mpxx_lite_malllocate_header_t*)((uintptr_t)p+totalsz);
		...
		*(uintptr_t*)GET_FOOTER_PTR(s) = (uintptr_t)s;
		...
		IFDBG3THEN {
		    ...
		    char u2abuf_a[MPXX_XTOA_HEX64_BUFSZ];
		    char u2abuf_b[MPXX_XTOA_HEX64_BUFSZ];
		    char u2abuf_c[MPXX_XTOA_HEX64_BUFSZ];
		    char u2abuf_d[MPXX_XTOA_HEX64_BUFSZ];

		    mpxx_u64toa((uintptr_t)p, u2abuf_a,u2abufsz,
			MPXX_XTOA_RADIX_HEX, NULL);
		    mpxx_u64toa((uintptr_t)s, u2abuf_b,u2abufsz,
			MPXX_XTOA_RADIX_HEX, NULL);
		    DBMS3( stdout, "p=0x%s s=0x%s" EOL_CRLF, u2abuf_a, u2abuf_b);
		     
		    mpxx_u64toa((uintptr_t)p->prev_p, u2abuf_a,u2abufsz,
			MPXX_XTOA_RADIX_HEX, NULL);
		    mpxx_u64toa((uintptr_t)p->next_p, u2abuf_b,u2abufsz,
			MPXX_XTOA_RADIX_HEX, NULL);
		    mpxx_u64toa((uintptr_t)s->prev_p, u2abuf_c,u2abufsz,
			MPXX_XTOA_RADIX_HEX, NULL);   
		    mpxx_u64toa((uintptr_t)s->next_p, u2abuf_d,u2abufsz,
			MPXX_XTOA_RADIX_HEX, NULL);
	...
	IFDBG3THEN {
	    ...
	    mpxx_u64toa((uintptr_t)retptr, u2abuf_a,u2abufsz,
		 MPXX_XTOA_RADIX_HEX, NULL);
...
static int region_pointer_check(mpxx_lite_mallocater_t *const self_p, const mpxx_lite_malllocate_header_t * const cur)
{
    const mpxx_lite_malllocate_header_t *p = cur->prev_p, *n = cur->next_p;
    int result = 0;
...
/**
 * @fn void mpxx_lite_mallocater_free_with_obj(mpxx_lite_mallocater_t *const self_p, void * const ptr)
...
{
    mpxx_lite_malllocate_header_t *cur;
    int result;
    const char *const myfunc=__FUNCTION__;
    DBMS5( stderr, "mpxx_lite_mallocater_free_with_obj : execute" EOL_CRLF);
...
/**
 * @fn void _mpxx_lite_mallocater_dump_region_list(void)
 * @brief 現在の領域確保マップ（リージョンテーブル）をダンプします。
 * @param self_p オブジェクトインスタンスポインタ
 **/
void _mpxx_lite_mallocater_dump_region_list(mpxx_lite_mallocater_t const *const self_p)
{
    volatile mpxx_lite_malllocate_header_t const *p; 
    unsigned int n;
    ...
    char u2abuf_b[MPXX_XTOA_DEC64_BUFSZ];
    char u2abuf_c[MPXX_XTOA_DEC64_BUFSZ];
    char u2abuf_d[MPXX_XTOA_DEC64_BUFSZ];

    DBMS5( stderr, "_mpxx_lite_mallocater_dump_region_list : execute" EOL_CRLF);

    mpxx_u64toa((uint64_t)((intptr_t)self_p), u2abuf_a,u2abufsz,
		    MPXX_XTOA_RADIX_HEX, NULL);
    mpxx_u64toa((uint64_t)((intptr_t)&self_p->base), u2abuf_b,u2abufsz,
		    MPXX_XTOA_RADIX_HEX, NULL);
	...
	flag = AREA_IS_ALLOC(p) ? (AREA_FOOTER_IS_NG(p) ? "allocNG" :"allocOK") : "free";
        mpxx_u64toa((uint64_t)((intptr_t)p), u2abuf_a,u2abufsz,
	    MPXX_XTOA_RADIX_HEX, NULL);
        mpxx_u64toa((uint64_t)((intptr_t)GET_HEAD2PTR(p)),u2abuf_b,u2abufsz,
	    MPXX_XTOA_RADIX_HEX, NULL);
	mpxx_u64toa((uint64_t)p->size, u2abuf_c,u2abufsz,
	    MPXX_XTOA_RADIX_DEC, NULL);
        mpxx_u64toa((uint64_t)((uintptr_t)&(p->next_p)),u2abuf_d,u2abufsz,
	    MPXX_XTOA_RADIX_HEX, NULL);
	...
	mpxx_u64toa((uint64_t)p->size, u2abuf_c,u2abufsz,
	    MPXX_XTOA_RADIX_DEC, NULL);
        mpxx_u64toa((uint64_t)((uintptr_t)&(p->prev_p)),u2abuf_d,u2abufsz,
	    MPXX_XTOA_RADIX_HEX, NULL);
	DMSG( stderr, dfmt, n, u2abuf_a, u2abuf_b, magic_is, u2abuf_c, flag, u2abuf_d);
    }
...
/**
 * @fn void *mpxx_lite_mallocater_realloc_with_obj(mpxx_lite_mallocater_t *const self_p, void *const ptr, const size_t size)
...
void *mpxx_lite_mallocater_realloc_with_obj(mpxx_lite_mallocater_t *const self_p, void *const ptr, const size_t size)
{
    ...
    if( NULL == ptr ) {
	return mpxx_lite_mallocater_alloc_with_obj(o, size);
    }
	...
	if( freearea > TOTALAREASIZE(1) ) {
	    ...
	    mpxx_lite_malllocate_header_t *new = (mpxx_lite_malllocate_header_t*)((intptr_t)cur+cur->size);
	...
	mpxx_lite_malllocate_header_t *n = (mpxx_lite_malllocate_header_t*)cur->next_p;
	/* 後方バッファが空であることを確認する */
	if(AREA_IS_FREE(n)) {
		...
		if( freearea == 0 || (freearea < TOTALAREASIZE(1))) {
		    ...
		    *(uintptr_t*)GET_FOOTER_PTR(cur) = (uintptr_t)cur;
		    ...
		    /* 後方エリアを再定義する */
		    n = (mpxx_lite_malllocate_header_t*)((uintptr_t)cur + totalsz);
		    n->size = freearea;
	...
	mpxx_lite_malllocate_header_t *b = (mpxx_lite_malllocate_header_t*)cur->prev_p;
	/* 前方バッファが空であることを確認する */
	if(AREA_IS_FREE(b)) {
	    const size_t totalsz = TOTALAREASIZE(size);
		...
		if( freearea > TOTALAREASIZE(0) ) {
		    ...
		    mpxx_lite_malllocate_header_t *new = (mpxx_lite_malllocate_header_t*)((intptr_t)cur + totalsz);
		    /* 最初にデータを移動する */
	...
	new_ptr = mpxx_lite_mallocater_alloc(size);
	if( NULL != new_ptr ) {
...
/**
 * @fn void *mpxx_lite_mallocater_alloc(const size_t size)
 * @brief CRL allocをエミュレートするためのラッパ関数
 *     注意） MMUがなくても利用可能なように、簡易な処理になっています。
 *	　　　バッファーオーバランについては mpxx_lite_mallocater_free()で一応確認します。
...
{
    mpxx_lite_mallocater_t * const o = get_lite_allocater_own();

    return mpxx_lite_mallocater_alloc_with_obj( o, size);
}

/**
 * @fn void mpxx_lite_mallocater_free(void *const ptr)
...
void mpxx_lite_mallocater_free(void *const ptr)
{
    ...
    mpxx_lite_mallocater_free_with_obj(o, ptr);
}

/**
 * @fn void *mpxx_lite_mallocater_realloc(void *const ptr, const size_t size)
 * @brief CRL reallocをエミュレートするためのラッパ関数
 *  ptrがNULL指定の場合はmpxx_lite_mallocater_alloc()と同等の処理をします。
...
void *mpxx_lite_mallocater_realloc(void *const ptr, const size_t size)
{
    ...
    return mpxx_lite_mallocater_realloc_with_obj( o, ptr, size);
}

/**
 * @fn int64_t mpxx_lite_mallocater_avphys_with_obj(void *const ptr)
...
{
    mpxx_lite_mallocater_t * const o = self_p;
    mpxx_lite_malllocate_header_t const *p; 
