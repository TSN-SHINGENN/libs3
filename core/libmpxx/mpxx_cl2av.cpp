...
static own_strtok_t own_strtok_reset(const mpxx_cl2av_attr_t attr)
...
extern int mpxx_cl2av_init(mpxx_cl2av_t *const self_p, const char *const delim, const char delim2, const mpxx_cl2av_attr_t *const  attr_p)
	__attribute__ ((optimize("Os")));
extern int mpxx_cl2av_execute( mpxx_cl2av_t *const self_p, const char *const sentence)
	__attribute__ ((optimize("Os")));
extern void mpxx_cl2av_destroy( mpxx_cl2av_t *const self_p)
	__attribute__ ((optimize("Os")));

extern int mpxx_cl2av_lite( char * const __restrict str, const char * __restrict delim, char delim2, int *const __restrict argc_p, char ** __restrict argv_p, const unsigned int limit, mpxx_cl2av_attr_t *const attr_p )
	__attribute__ ((optimize("Os")));
...
/**
 * @fn int mpxx_cl2av_init(mpxx_cl2av_t *const self_p, const char *const delim, const char delim2, const mpxx_cl2av_attr_t *const  attr_p)
 * @brief gof_cmd_makeargvオブジェクトを初期化します
 * @param self_p mpxx_cl2av_tインスタンスポインタ
 * @param delim デリミタのASCII文字集合列( NULL で " "と等価 )
 * @param delim2 囲い文字ASCII文字('\0'で無視)
 * @param attr_p mpxx_cl2av_attr_t属性ポインタ(NULLでデフォルト動作)
 * @retval 0 成功
...
int mpxx_cl2av_init(mpxx_cl2av_t *const self_p, const char *const delim, const char delim2, const mpxx_cl2av_attr_t *const  attr_p)
{
    ...
    memset(self_p, 0x0, sizeof(mpxx_cl2av_t));
    ...
    e = (cl2av_ext_t *)
	mpxx_malloc(sizeof(cl2av_ext_t));
    if (NULL == e) {
	status = ENOMEM;
	goto out;
    }
    ...
    result = mpxx_stl_vector_init(&e->argvec, sizeof(char *));
    if (result) {
	status = result;
	goto out;
    } 
    ...
    result = mpxx_stl_vector_reserve(&e->argvec, 16);
    if (result) {
	status = result;
	goto out;
    }
	...
	if (!(isalnum((int) delim2) || ispunct((int) delim2))) {
	    DBMS3(stderr, "mpxx_cl2av_init : delim2 is not ascii=%c" EOL_CRLF, delim2);
	    status = EACCES;
	    goto out;
	}
	e->delim2 = delim2;
	...
	e->delim = (char *) mpxx_malloc(1 + strlen(std_delim));
	if (NULL == e->delim) {
	    status = ENOMEM;
	    goto out;
	}
	...
	e->delim = (char *) mpxx_malloc(1 + strlen(delim));
	if (NULL == e->delim) {
	    status = ENOMEM;
	    goto out;
	}
  ...
  out:
    if (status) {
	mpxx_cl2av_destroy(self_p);
    }
    return status;
}

/**
 * @fn void mpxx_cl2av_destroy(mpxx_cl2av_t *const self_p)
...
void mpxx_cl2av_destroy(mpxx_cl2av_t *const self_p)
{
    ...
    if (e->init.f.argvec) {
	mpxx_stl_vector_destroy(&e->argvec);
	e->init.f.argvec = 0;
    }
    ...
    if (NULL != e->work_buf) {
	mpxx_free(e->work_buf);
    }

    if (e->init.flags) {
	DBMS1(stderr,
	      "mpxx_cl2av_destroy : e->init.flags=0x%llx" EOL_CRLF,
...
/**
 * @fn static own_strtok_t own_strtok_reset( const mpxx_cl2av_attr_t attr)
...
/**
 * @fn int mpxx_cl2av_execute( mpxx_cl2av_t *const self_p, const char *const sentence)
...
int mpxx_cl2av_execute( mpxx_cl2av_t *const self_p, const char *const sentence)
{
    ...
    DBMS3(stderr, "mpxx_cl2av_execute : sentence =%s" EOL_CRLF, sentence);

    /* initalize */
    mpxx_stl_vector_clear(&e->argvec);
    mpxx_stl_vector_reserve(&e->argvec, 16);
    self_p->argc = 0;

    new_buf = (char *) mpxx_realloc(e->work_buf, strlen(sentence) + 1);
    if (NULL == new_buf) {
	return ENOMEM;
    }
	...
	if (result) {
	    DBMS1(stderr, "mpxx_cl2av_execute : own_strtok fail" EOL_CRLF);
	    return result;
	}
	...
	DBMS5(stderr, "mpxx_cl2av_execute : argp(%p)=%s" EOL_CRLF, argp,
	      argp);

	result =
	    mpxx_stl_vector_push_back(&e->argvec, &argp, sizeof(argp));
	if (result) {
	    DBMS1(stderr,
		  "mpxx_cl2av_execute : mpxx_stl_vector_push_back fail" EOL_CRLF);
	    return result;
	}

    }

    self_p->argc = mpxx_stl_vector_size(&e->argvec);
    self_p->argv = (char **)mpxx_stl_vector_ptr_at(&e->argvec, 0);

    return 0;
}

/**
 * @fn int mpxx_cl2av_lite( char * const __restrict str, const char * __restrict delim, char delim2, int *const __restrict argc_p, char ** __restrict argv_p, const unsigned int limit, mpxx_cl2av_attr_t *const attr_p )
...
int mpxx_cl2av_lite( char * const __restrict str, const char * __restrict delim, char delim2, int *const __restrict argc_p, char ** __restrict argv_p, const unsigned int limit, mpxx_cl2av_attr_t *const attr_p )
{
    ...
    if (delim2 != '\0') {
	if (!(isalnum((int) delim2) || ispunct((int) delim2))) {
	    DBMS3(stderr, "mpxx_cl2av_lite : delim2 is not ascii=%c" EOL_CRLF, delim2);
	    return EACCES;
	}
    }
	...
	if(result) {
	    DBMS1( stderr, "mpxx_cl2av_lite : own_strtok fail, strerror:%s" EOL_CRLF, strerror(result));
	    return result;
	}
	if( argp == NULL ) {
	    break;
