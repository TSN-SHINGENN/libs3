...
#define get_stl_list_ext(s) MPXX_STATIC_CAST(stl_list_ext_t*,(s)->ext)
#define get_const_stl_list_ext(s) MPXX_STATIC_CAST(const stl_list_ext_t*,(s)->ext)

#if defined(__GNUC__) && defined(_LIBS3_MICROCONTROLLER_ENABLE_GCC_OPTIMIZE_FOR_SIZE )

static queitem_t *list_search_item(stl_list_ext_t *const e,
				    const size_t num)
	__attribute__ ((optimize("Os")));

extern int mpxx_stl_list_pop_front(mpxx_stl_list_t *const)
	__attribute__ ((optimize("Os")));
extern int mpxx_stl_list_remove_at(mpxx_stl_list_t *const, const size_t)
	__attribute__ ((optimize("Os")));
extern int mpxx_stl_list_push_back(mpxx_stl_list_t *const, const void *const, const size_t)
	__attribute__ ((optimize("Os")));
extern int mpxx_stl_list_insert(mpxx_stl_list_t *const, const size_t, const void *const, const size_t)
	__attribute__ ((optimize("Os")));
extern int mpxx_stl_list_remove_at(mpxx_stl_list_t *const, const size_t)
	__attribute__ ((optimize("Os")));
extern int mpxx_stl_list_overwrite_element_at( mpxx_stl_list_t *const, const size_t, const void *const, const size_t)
	__attribute__ ((optimize("Os")));
#else /* Normal */
static queitem_t *list_search_item(stl_list_ext_t *const e,
				    const size_t num);
...
/**
 * @fn int mpxx_stl_list_init( mpxx_stl_list_t *const self_p, const size_t sizof_element)
 * @brief 双方向キューオブジェクトを初期化します
 * @param self_p mpxx_stl_list_t構造体インスタンスポインタ
 ...
 */
int mpxx_stl_list_init(mpxx_stl_list_t *const self_p,
			   const size_t sizof_element)
{
    stl_list_ext_t *e = NULL;
    memset(self_p, 0x0, sizeof(mpxx_stl_list_t));

    e = (stl_list_ext_t *)
	mpxx_malloc(sizeof(stl_list_ext_t));
    if (NULL == e) {
	DBMS1(stderr, "mpxx_stl_list_init : mpxx_malloc(ext) fail" EOL_CRLF);
	return EAGAIN;
    }
    memset(e, 0x0, sizeof(stl_list_ext_t));
...
/**
 * @fn int mpxx_stl_list_destroy( mpxx_stl_list_t *const self_p )
...
int mpxx_stl_list_destroy(mpxx_stl_list_t *const self_p)
{
    ...
    result = mpxx_stl_list_clear(self_p);
    if (result) {
	DBMS1(stderr, "mpxx_stl_list_destroy : mpxx_stl_list_clear fail" EOL_CRLF);
	return EBUSY;
    }
...
/**
 * @fn int mpxx_stl_list_push_back(mpxx_stl_list_t *const self_p, const void *const el_p, const int sizof_element)
 * @brief 双方向キューの後方にエレメントを追加します
 * @param self_p mpxx_stl_list_t構造体インスタンスポインタ
 * @param el_p エレメントポインタ
 * @param sizof_element エレメントサイズ(mpxx_stl_list_initで指定した以外のサイズはエラーとします)
 * @retval 0 成功
 * @retval EINVAL エレメントサイズが異なる
 * @retval EAGAIN リソースの獲得に失敗
 * @retval -1 それ以外の致命的な失敗
 */
int mpxx_stl_list_push_back(mpxx_stl_list_t *const self_p,
				const void *const el_p,
				const size_t sizof_element)
{
    int status;
    stl_list_ext_t *const e = get_stl_list_ext(self_p);
    queitem_t *__restrict f = NULL;

    DBMS3(stderr, "mpxx_stl_list_push_back : execute" EOL_CRLF);

    if (e->cnt == 0) {
	return mpxx_stl_list_insert(self_p, 0, el_p, sizof_element);
    }
    ...
    f = (queitem_t *) mpxx_malloc(sizeof(queitem_t) + e->sizof_element);
    if (NULL == f) {
	DBMS1(stderr, "mpxx_stl_list_push : mpxx_malloc(queitem_t) fail" EOL_CRLF);
	status = EAGAIN;
	goto out;
    }

    memset(f, 0x0, sizeof(queitem_t));
    mpxx_memcpy(f->data, el_p, e->sizof_element);
    f->prev = e->base.prev;
    f->next = &e->base;
...
/**
 * @fn int mpxx_stl_list_push_front(mpxx_stl_list_t *const self_p, const void *const el_p, const size_t sizof_element)
 ...
 * @retval -1 それ以外の致命的な失敗
 */
int mpxx_stl_list_push_front(mpxx_stl_list_t *const self_p,
				 const void *const el_p,
				 const size_t sizof_element)
{
    return mpxx_stl_list_insert(self_p, 0, el_p, sizof_element);
...
/**
 * @fn int mpxx_stl_list_pop_front(mpxx_stl_list_t *self_p)
...
/**
 * @fn int mpxx_stl_list_pop_back(mpxx_stl_list_t *const self_p)
...
/**
 * @fn int mpxx_stl_list_clear( mpxx_stl_list_t *const self_p )
...
int mpxx_stl_list_clear(mpxx_stl_list_t *const self_p)
{
    ...
    for (i = e->cnt; i != 0; --i) {
	result = mpxx_stl_list_pop_front(self_p);
	if (result) {
	    IFDBG1THEN {
		...
		DBMS1(stderr,
		  "mpxx_stl_list_clear : mpxx_stl_list_pop_front[%s] fail" EOL_CRLF, xtoa_buf);
	    }
...
/**
 * @fn int mpxx_stl_list_get_pool_cnt( mpxx_stl_list_t *const self_p );
...
{
    const stl_list_ext_t *const e = get_const_stl_list_ext(self_p);

    return e->cnt;
}

/**
 * @fn int mpxx_stl_list_is_empty( mpxx_stl_list_t *self_p)
...
/**
 * @fn static queitem_t *list_search_item( stl_list_ext_t *const e, const size_t num)
 * @brief numからqueitemのポインタを検索します
 * @param e stl_list_ext_t構造体ポインタ
...
/**
 * @fn int mpxx_stl_list_get_element_at( mpxx_stl_list_t *const self_p, const int num, void *const el_p, const size_t sizof_element);
 ...
 */
int mpxx_stl_list_get_element_at(mpxx_stl_list_t *const self_p,
				     ...
				     const size_t sizof_element)
{
    ...
    if (mpxx_stl_list_is_empty(self_p)) {
	return EACCES;
    }
    ...
    mpxx_memcpy(el_p, item_p->data, e->sizof_element);

    return 0;
}

/**
 * @fn int mpxx_stl_list_remove_at(mpxx_stl_list_t *const self_p, const size_t num)
 ...
 * @retval EINVAL sizof_elementのサイズが異なる(小さい）
 */
int mpxx_stl_list_remove_at(mpxx_stl_list_t *const self_p,
			       const size_t num)
{
    stl_list_ext_t *const e = get_stl_list_ext(self_p);
...
/**
 * @fn void mpxx_stl_list_dump_element_region_list( mpxx_stl_list_t *const self_p)
 * @brief 双方向キューに保存されている要素の情報をダンプします
 *	主にこのデバッグ用
 * @param self_p mpxx_stl_list_t構造体インスタンスポインタ
 **/
void mpxx_stl_list_dump_element_region_list(mpxx_stl_list_t *const
						self_p)
{
    const stl_list_ext_t *const e = get_const_stl_list_ext(self_p);
...
__attribute__ ((unused))
    ...
    DMSG(stdout,
	    "mpxx_stl_list_dump_element_region_list : execute\n");
    mpxx_u64toadec( (uint64_t)e->sizof_element, xtoa_buf, xtoa_bufsz);
    DMSG(stderr, "sizof_element = %s" EOL_CRLF, xtoa_buf);
    mpxx_u64toadec( (uint64_t)e->cnt, xtoa_buf, xtoa_bufsz);
    DMSG(stderr, "numof_elements = %s" EOL_CRLF, xtoa_buf);
    mpxx_i64toadec( (int64_t)e->start_id, xtoa_buf, xtoa_bufsz);
...
/**
 * @fn int mpxx_stl_list_insert( mpxx_stl_list_t *const self_p, const size_t num, const void *const el_p, const size_t sizof_element )
 * @brief 双方向キューの指定されたエレメント番号の直前に、データを挿入します.
 *	numに0を指定した場合は、mpxx_listue_push_front()と同等の動作をします。
 * @param self_p mpxx_stl_list_t構造体インスタンスポインタ
 * @param num 0から始まるキュー先頭からのエレメント配列番号
 * @param el_p エレメントデータポインタ
 * @param sizof_element エレメントサイズ(主にエレメントサイズ検証向け)
 ...
 * @retval EAGAIN リソースの獲得に失敗
 */
int mpxx_stl_list_insert(mpxx_stl_list_t *const self_p,
			     ...
			     const size_t sizof_element)
{
	...
	mpxx_u64toadec( (uint64_t)num, xtoa_buf_a, xtoa_bufsz);
	mpxx_u64toadec( (uint64_t)e->cnt, xtoa_buf_b, xtoa_bufsz);
	DBMS3(stderr, "mpxx_stl_list_insert : execute" EOL_CRLF);
	DBMS3(stderr, "mpxx_stl_list_insert : num=%s e->cnt=%s" EOL_CRLF,
	...
	IFDBG3THEN {
	    ...
	    DBMS3(stderr,
	      "mpxx_stl_list_insert : e->sizof_element=%s sizof_element=%s" EOL_CRLF, xtoa_buf_a, xtoa_buf_b);
	}
	return EINVAL;
    }
    ...
    DBMS3(stderr, "mpxx_stl_list_insert : front=0x%p e->base=0x%p" EOL_CRLF,
	  front, &e->base);

    f = (queitem_t *) mpxx_malloc(sizeof(queitem_t) + e->sizof_element);
    if (NULL == f) {
	DBMS1(stderr, "mpxx_stl_list_push : mpxx_malloc(queitem_t+element) fail" EOL_CRLF);
	status = EAGAIN;
	goto out;
    }
...
/**
 * @fn int mpxx_stl_list_overwrite_element_at( mpxx_stl_list_t *const self_p,  const size_t num, const void *const el_p, const size_t sizof_element)
...
int mpxx_stl_list_overwrite_element_at( mpxx_stl_list_t *const self_p,  const size_t num, const void *const el_p, const size_t sizof_element)
{
    ...
    mpxx_memcpy(item_p->data, el_p, e->sizof_element);

    return 0;
}

/**
 * @fn int mpxx_stl_list_front( mpxx_stl_list_t *const self_p, const void *const el_p, const size_t sizeof_element)
...
int mpxx_stl_list_front( mpxx_stl_list_t *const self_p, void *const el_p, const size_t sizof_element)
{
    ...
    mpxx_memcpy(el_p, tmp->data, e->sizof_element);
...
/**
 * @fn int mpxx_stl_list_back( mpxx_stl_list_t *const self_p, void *const el_p, const size_t sizof_element)
