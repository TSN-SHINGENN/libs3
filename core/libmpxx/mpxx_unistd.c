...
#include "libmbmcs_libmpxx_bridge.h"
#endif
...
extern unsigned int mpxx_sleep( const unsigned int )
		__attribute__ ((optimize("Os")));
extern void mpxx_msleep( const unsigned int )
		__attribute__ ((optimize("Os")));
extern int64_t mpxx_sysconf( const enum_mpxx_sysconf_t name )
		__attribute__ ((optimize("Os")));
extern int mpxx_is_user_an_admin(void)
		__attribute__ ((optimize("Os")));
extern int mpxx_get_username( char*, const size_t)
		__attribute__ ((optimize("Os")));
extern int mpxx_get_hostname( char*, const size_t)
		__attribute__ ((optimize("Os")));
int mpxx_fork(void)
		__attribute__ ((optimize("Os")));
...
/**
 * @fn unsigned int mpxx_sleep(const unsigned int seconds)
 * @brief 指定秒間休止する。
...
unsigned int mpxx_sleep(const unsigned int seconds)
{
    ...
    return ~0;
#else
#error not impliment mpxx_sleep()
#endif
}


/**
 * @fn void mpxx_msleep(const unsigned int milisec)
 * @brief 指定ミリ秒間休止する。
 * @param milisec ミリ秒
 *	シグナルによって割り込まれることがあります。
 */
void mpxx_msleep(const unsigned int milisec)
{
...
/**
 * @fn int mpxx_sysconf(const enum_mpxx_sysconf_t name)
 * @brief 動作中の設定情報を取得する。
 * @param name enum_mpxx_sysconf_tで定義されたシステム情報名
 * @retval 0以上 MPXX_SYSCNF_PAGESIZEが指定された場合はメモリのページサイズ(バイト）
 * @retval 0以上 MPXX_SYSCNF_NPROCESSORS_CONFが指定された場合は設定されたプロセッサコア数
 * @retval 0以上 MPXX_SYSCNF_NPROCESSORS_ONLNが指定された場合は現在有効なプロセッサコア数
 * @retval 0以上 MPXX_SYSCNF_PHYS_PAGESが指定された場合はOSが認識している物理メモリのページ数
 * @retval 0以上 MPXX_SYSCNF_AVPHYS_PAGESが指定された場合は現在利用可能な物理メモリのページ数
 * @retval -1 失敗(errno = ENOTSUP その挙動はサポートされていない（実装できていない))
 */
int64_t mpxx_sysconf(const enum_mpxx_sysconf_t name)
{
#if defined( MPXX_SYSCONF_NOT_SUPPORT)
    DMSG( stderr, "not supported %s function" EOL_CRLF, __func__);
    abort();
#else /* else MPXX_SYSCONF_NOT_SUPPORT */
    switch (name) {
    case MPXX_SYSCNF_PAGESIZE:
#ifdef WIN32
	{
	    SYSTEM_INFO info;
	    GetSystemInfo(&info);
	...
	DMSG(stderr, "not impliment mpxx_sysconf(MPXX_SYSCNF_PAGESIZE) on MICROBLAZE" EOL_CRLF);
	errno = ENOTSUP;
	return -1;
#else
#error  'not impliment mpxx_sysconf(MPXX_SYSCNF_PAGESIZE)'
#endif
	break;
    case MPXX_SYSCNF_NPROCESSORS_CONF:
#ifdef WIN32
	{
	    SYSTEM_INFO info;
	    GetSystemInfo(&info);
...
#else
	// not impliment
#error  'not impliment mpxx_sysconf(MPXX_SYSCNF_NPROCESSORS_CONF)'
	return -1;
#endif
	break;
    case MPXX_SYSCNF_NPROCESSORS_ONLN:
#ifdef WIN32
	{
	    int v = 0;
	    unsigned int i;
...
#else				/*  qnx 6.3.2 or more */
	{
	    ...
	    rsizep = freep = mpxx_malloca(size_tot);
	...
	return 1;
#else				/* other */
#error  'not impliment mpxx_sysconf(MPXX_SYSCNF_NPROCESSORS_ONLN)'
	return -1;
#endif
	break;
    case MPXX_SYSCNF_PHYS_PAGES:
...
#elif defined(WIN32)
	{
		...
		DBMS1(stderr,
		      "mpxx_sysconf : GlobalMemoryStatusEx fail" EOL_CRLF);
		return -1;
	    }
...
#elif defined(Darwin)
	{
	    ...
	    if (pagesize < 0) {
		DBMS1(stderr,
		      "mpxx_sysconf : sysconf(_SC_PAGESIZE) fail" EOL_CRLF);
		return -1;
	    }
		...
		DBMS1(stderr, "mpxx_sysconf : host_info fail" EOL_CRLF);
		return -1;
	    }
	...
	/* ブロックメモリをメインメモリとする場合は、物理メモリーをページとして管理していない */
	/* 挙動を調べて値を正しくセットする必要がある */
	DMSG(stderr, "not impliment mpxx_sysconf(MPXX_SYSCNF_PHYS_PAGES) on MICROBLAZE" EOL_CRLF);
	return -1;
#else				/* other */
#error  'not impliment mpxx_sysconf(MPXX_SYSCNF_PHYS_PAGES)'
#endif
	break;
    case MPXX_SYSCNF_AVPHYS_PAGES:
#if defined(Linux)
	/* posix standard */
...
#elif defined(Darwin)
	{
		...
		DBMS1(stderr, "mpxx_sysconf : host_statistics fail" EOL_CRLF);
		return -1;
	    }
	...
	/* ブロックメモリをメインメモリとする場合は、物理メモリーをページとして管理していない */
	/* 挙動を調べて値を正しくセットする必要がある */
	DMSG(stderr, "not impliment mpxx_sysconf(MPXX_SYSCNF_AVPHYS_PAGES) on MICROBLAZE" EOL_CRLF);
	return -1;
#else				/* other */
#error  'not impliment mpxx_sysconf(MPXX_SYSCNF_AVPHYS_PAGES)'
#endif
    default:
	return -1;
    }
    ...
    return -1;
#endif /* end of MPXX_SYSCONF_NOT_SUPPORT */
}

/**
 * @fn int mpxx_is_user_an_admin(void)
...
int mpxx_is_user_an_admin(void)
{
    ...
    return 1;
#else
#error  'can not defined mpxx_is_user_an_admin()'
#endif
}

/**
 * @fn int mpxx_fork(void)
...
int mpxx_fork(void)
{
#if (defined(MICROBLAZE) && defined(STANDALONE)) || defined(MPXX_FORK_NOT_SUPPORT)
	DMSG( stderr, "not supported %s function \r\n", __func__);
	errno = ENOSYS;
	return -1;
#else /* MPXX_FORK_NOT_SUPPORT */
    ...
    return -1;
#else /* others OS */
#error  'can not defined mpxx_fork()'
#endif
#endif /* end of MPXX_FORK_NOT_SUPPORT */
}

/**
 * @fn int mpxx_get_username(char *name_p, const size_t maxlen)
...
#else /* MPXX_GET_USERNAME_NOT_SUPPORT */

    return ENOSYS;
#if 0
#error  'can not defined mpxx_get_username()'
    return ENOSUP;
#endif
#endif
}
...
/**
 * @fn int mpxx_get_hostname( char * name, const size_t maxlen)
...
int mpxx_get_hostname( char *name, const size_t maxlen)
{
    ...
    return EFAULT;
#else /* MPXX_GET_HOSTNAME_NOT_SUPPORT */

    return ENOSYS;
#if 0
#error  'can not defined mpxx_get_hostname()'
    return ENOSUP;
#endif
#endif
